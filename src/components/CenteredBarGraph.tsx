...
export interface CenteredBarGraphProps {
...
/**
 * CenteredBarGraph Component
 *
 * Renders generic signed graph data around a centered zero axis.
 *
 ...
 */
export function CenteredBarGraph({ title, data, roundTo, sortDirection = 'des' }: CenteredBarGraphProps) {
  ...
  const maxAbsVal = Math.max(...data.map((point) => Math.abs(point.value)), 0);
        ...
        {sortedData.map((point) => {
          ...
          return (
            ...
            >
              ...
              <div className="graph-track centered">
                <div
                  className={`graph-fill centered ${point.isHomeTeam ? 'home' : 'away'} ${isPositive ? 'positive' : 'negative'}`}
                  style={{
                    width: `${widthPercentage}%`,
                    left: isPositive ? '50%' : `calc(50% - ${widthPercentage}%)`,
                  }}
                >
                  <span className={`graph-value-label centered ${isPositive ? 'positive' : 'negative'} ${widthPercentage > 15 ? 'inside' : 'outside'}`}>
